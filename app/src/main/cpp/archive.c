...
static bool fill_entry_zero_copy(struct archive *arc, entry *ent) {
    void *buffer = NULL;
    size_t buffer_size = 0;
    la_int64_t output_ofs = 0;
    archive_read_data_block(arc, (const void **) &buffer, &buffer_size, &output_ofs);
    bool zero_copy = ADDR_IN_FILE_MAPPING(buffer) && !output_ofs && buffer_size == ent->size;
    ent->addr = zero_copy ? buffer : NULL;
    return zero_copy;
}
...
Java_com_hippo_ehviewer_jni_ArchiveKt_extractToByteBuffer(JNIEnv *env, jclass thiz, jint index) {
    ...
    if (ent->addr) {
        return (*env)->NewDirectByteBuffer(env, ent->addr, size);
    } else {
        archive_ctx *ctx = NULL;
        if (!archive_get_ctx(&ctx, ent->index)) {
