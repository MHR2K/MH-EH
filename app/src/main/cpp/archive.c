...
static int stream_pread(uint8_t *dst, la_int64_t off, size_t len) {
    ...
    while (got < len) {
        ...
        stream_bytes_read += n;
        // Do NOT break on n < chunk: SMB/ReadAhead may return partial reads
        // that are not EOF. Only n==0 (null arr) exits the loop.
    }
    return (int) got;
}
...
/**
 * Stream-aware extract: uses the caller-provided [bridge] for seek/read
 * instead of the global [g_stream_bridge]. Thread-safe when each caller
 * passes its own bridge instance.
 */
JNIEXPORT jobject JNICALL
Java_com_hippo_ehviewer_jni_ArchiveKt_extractStreamToByteBuffer(
        JNIEnv *env, jclass thiz, jint index, jobject bridge) {
    EH_UNUSED(thiz);
    if (!bridge) return 0;
    pthread_mutex_lock(&stream_mutex);
    if (index < 0 || (size_t) index >= entryCount || !entries || archive_should_abort()) {
    ...
    ssize_t size = ent->size;
    if (ent->addr) {
        jobject result = (*env)->NewDirectByteBuffer(env, ent->addr, size);
        pthread_mutex_unlock(&stream_mutex);
        return result;
    }
    ...
    if (use_stream_io && use_zip_cd_index) {
        // Save globals, override with caller's bridge for this extract
        jobject saved_bridge = g_stream_bridge;
        jmethodID saved_mid_read = g_mid_read;
        jmethodID saved_mid_seek = g_mid_seek;
        la_int64_t saved_pos = g_stream_pos;

        jclass cls = (*env)->GetObjectClass(env, bridge);
        jmethodID mid_read = (*env)->GetMethodID(env, cls, "nativeRead", "(I)[B");
        jmethodID mid_seek = (*env)->GetMethodID(env, cls, "nativeSeek", "(JI)J");
        (*env)->DeleteLocalRef(env, cls);
        if (!mid_read || !mid_seek) {
            LOGE("%s", "extractStreamToByteBuffer: bridge methods missing");
            release_decode_buffer(addr);
            pthread_mutex_unlock(&stream_mutex);
            return 0;
        }
        g_stream_bridge = bridge;
        g_mid_read = mid_read;
        g_mid_seek = mid_seek;
        g_stream_pos = 0;

        if (zip_stream_extract_entry(ent, addr, (size_t) size) == 0) {
            result = (*env)->NewDirectByteBuffer(env, addr, size);
        } else {
            LOGE("%s%d", "ZIP stream extract failed for index ", index);
            release_decode_buffer(addr);
        ...
        // Restore globals
        g_stream_bridge = saved_bridge;
        g_mid_read = saved_mid_read;
        g_mid_seek = saved_mid_seek;
        g_stream_pos = saved_pos;
    } else {
        LOGE("%s", "extractStreamToByteBuffer: not a stream ZIP index");
        release_decode_buffer(addr);
    }
